...
class MySqlSchemaExplorer:
    ...
    @staticmethod
    def get_database_connection(conn_info: ConnectionInfo) -> MySQLConnection:
    ...
    @contextmanager
    def open_database(self, conn_info: ConnectionInfo, context: str) -> MySQLConnection:
        """
        ...
        try:
            conn = self.get_database_connection(conn_info)
    ...
    @contextmanager
    def open_database_cursor(self, conn_info: ConnectionInfo, context: str, named_tuple: bool = False,
            dictionary: bool = False):
        cursor: MySQLCursor

        with self.open_database(conn_info, context) as db:
            try:
                if named_tuple:
    ...
    @staticmethod
    def _extract_tables(cursor: MySQLCursorDict, db: Database):
    ...
    @staticmethod
    def extract_columns(cursor: MySQLCursorDict, db_name: str, table: Table):
        ...
        for index, entry in enumerate(rows):
            ...
            column = Column(order=order, name=name, type=data_type, length=length, default=default,
                            is_nullable=is_nullable, is_key=is_key, is_unique=is_unique)
            table.columns[column.name] = column

    def extract(self, conn_info: ConnectionInfo) -> Database | None:
        db = Database(name=conn_info.database)

        cursor: MySQLCursorDict
        with self.open_database_cursor(conn_info, '', dictionary=True) as cursor:
